! function() {
    ...
    t.prototype.init = function() {
        ...
        if (this.disabled(this.conf.disabled), this.valRange && (this.conf.values = s(this.conf)), this.values.start = 0, this.values.end = this.conf.range ? this.conf.values.length - 1 : this.conf.CreditValue, this.conf.set && this.conf.set.length && n(this.conf)) {
            var t = this.conf.set;
            this.conf.range ? (this.values.start = this.conf.values.indexOf(t[0]), this.values.end = this.conf.set[1] ? this.conf.values.indexOf(t[0]) : null) : this.values.end = this.conf.values.indexOf(t[0])
        }
        ...
        for (var e = 0, s = this.conf.values.length; e < s; e++) {
            var n = i("span"),
                l = i("ins");
            n.appendChild(l), this.scale.appendChild(n), n.style.width = e === s - 1 ? 0 : this.step + "px", this.conf.labels ? l.innerHTML = '': 0 !== e && e !== s - 1 || (l.innerHTML = this.conf.values[e]), l.style.marginLeft = l.clientWidth / 2 * -1 + "px"
        }
    ...
    var i = function(t, i, e) {
        ...
        s = function(t) {
            ...
            return i.indexOf(t.values.max) < 0 && i.push('25 000' + ' €' + '&nbsp;&nbsp;'), i
        },
        n = function(t) {
